...
def yield_surface(M, pc, theta, puntos):
    ...
    fig.update_layout(
        showlegend=False,
        width=700,
        scene=dict(xaxis_title="σ1", yaxis_title="σ2", zaxis_title="σ3"),
...
def deformation(N, M, λ, k, G, pc, Δp, p, Δq, n, e, test):
    ...
    # elastic deformation
    matrix_elastic = array([[k / (v * p), 0], [0, 1 / (3 * G)]])
    ...
    if test == "Drained":
        matrix_plastic = (
            (λ - k)
            / (v * p * (M**2 + n**2))
            * array([[M**2 - n**2, 2 * n], [2 * n, 4 * n**2 / (M**2 - n**2)]])
        )
...
def cam_clay_undrained(M, λ, k, specimen, Points, Condition):
    ...
    for i in range(2, Points):
        ...
        Δq = q[i] - q[i - 1]
        e[i] = deformation(
            N, M, λ, k, G, pc[i], Δp, p[i], Δq, n[i], e[i - 1], Condition
        )
...
def p_vs_q(M, results):
    ...
    fig.add_trace(
        go.Scatter(
            x=[0, dff.loc[dff.index[-1], "p'"] * 1.2],
            y=[0, M * dff.loc[dff.index[-1], "p'"] * 1.2],
            mode="lines",
            name="Critical State Line (CSL)",
        )
    )
    fig.update_layout(
        title=dict(
            text="Mean Effective Stress vs Deviatoric Stress",
            x=0.5,
            xanchor="center",
            font=dict(size=24),
        ),
...
def q_vs_ε(M, results):
    ...
    fig.update_layout(
        title=dict(
            text="Deformation vs Deviatoric Stress",
            x=0.5,
            xanchor="center",
            font=dict(size=24),
        ),
...
def u_vs_ε(M, results):
    ...
    fig.update_layout(
        title=dict(
            text="Deformation vs Excess Pore Pressure ",
            x=0.5,
            xanchor="center",
            font=dict(size=24),
        ),
